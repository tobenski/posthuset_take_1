<x-admin-layout>
    <main class="py-12 w-screen h-screen overflow-y-scroll flex items-center justify-center">
        <div class="flex flex-col w-10/12 min-h-full">
            ...
            @endif
            <div class="mb-4">
                <a href="{{ route('admin.anbefalerMenu.create') }}" class="btn btn-primary">{{ __('Opret Ny anbefalermenu') }}</a>
            </div>
            ...
            <table class="w-full table-fixed">
                ...
                <tbody>
                    @foreach($anbefalerMenuer as $menu)
                        <tr>
                            ...
                            <td class="border px-4 py-2">
                                <div class="flex h-full">
                                    <a href="{{ route('admin.anbefalerMenu.edit', $menu) }}" class="btn btn-primary mr-2">{{ __('Edit') }}</a>
                                    <form method="POST" action="{{ route('admin.anbefalerMenu.destroy', $menu) }}" class="">
                                        @csrf
                                        @method('delete')
