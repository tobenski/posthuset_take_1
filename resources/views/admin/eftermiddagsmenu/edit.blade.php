<x-admin-layout>
    <main class="py-12 w-screen min-h-screen flex items-center justify-center">
        <div class="flex flex-col w-10/12 min-h-full">
            ...
            @endif
            <div>
                <h2 class="text-2xl font-bold">{{ __('Edit Eftermiddagsmenu') }}</h2>
                <form action="{{ route('admin.eftermiddagsMenu.update', $eftermiddagsmenu) }}" method="POST" enctype="multipart/form-data" >
                    @csrf
                    @method('put')
                    <div class="grid grid-cols-3 gap-6">
                        <label class="block">
                            ...
                            <input
                                ...
                                value="{{ $eftermiddagsmenu->firstday }}"
                                name="firstday"
                                id="firstday"
                            />
                        ...
                        <label class="block">
                            ...
                            <input
                                type="date"
                                class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0"
                                value="{{ $eftermiddagsmenu->lastday  }}"
                                name="lastday"
                                id="lastday"
                            />
                                ...
                                <div>
                                    <label class="inline-flex items-center">
                                        <input
                                            ...
                                            name="online"
                                            id="online"
                                            @if($eftermiddagsmenu->online) checked @endif />
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <label class="block">
                            ...
                            <input
                                type="text"
                                class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0"
                                value="{{ $eftermiddagsmenu->timeframe }}"
                                name="timeframe"
                                id="timeframe"
                            />
                        ...
                        <label class="block">
                            ...
                            <input
                                type="text"
                                class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0"
                                value="{{ $eftermiddagsmenu->comment }}"
                                name="comment"
                                id="comment"
                            />
                        ...
                        <div class="flex justify-start">
                            @if($eftermiddagsmenu->getFirstMedia('menu'))
                                {{ $eftermiddagsmenu->getFirstMedia('menu')->img()->attributes(['class' => 'w-36 m-6']) }}
                            @endif
                            <label class="block">
                ...
                <table class="w-full table-fixed mt-4 mb-12">
                    ...
                    <tbody>
                        <tr>
                            <form action="{{ route('admin.eftermiddagsMenu.afternoonDish.store', $eftermiddagsmenu) }}" method="POST">
                                @csrf
                    ...
                    @foreach($eftermiddagsmenu->retter as $ret)
                        <tr>
                            <form action="{{ route('admin.afternoonDish.update', $ret) }}" method="POST">
                                @csrf
                                @method('put')
                                <td class="px-4 py-2 border-2 border-gray-600">
                                    <input
